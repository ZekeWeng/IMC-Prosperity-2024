...
class Trader:
    ...
    PRODUCTS = {
    'STARFRUIT': {"TIME": [], "DATA": [], "DELTAS": [0], "PRICE": 0, "PLIMIT": 20, "STRATEGY": "LR", "LR_SIZE": 8},
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for symbol in state.order_depths:
            ...
            if len(self.PRODUCTS[symbol]["TIME"]) > self.PRODUCTS[symbol]["LR_SIZE"]:
                self.PRODUCTS[symbol]["PRICE"] = self.compute_price_regression(symbol, state.timestamp)

            if symbol in state.position.keys():
                logger.print("Current Quantity: " + str(state.position[symbol]))
            else:
                logger.print("Current Quantity: 0")
            ...
            # logger.print("Current Delta: " + str(self.PRODUCTS[symbol]["DELTAS"][-1]))
            ...
            # elif self.PRODUCTS[symbol]["STRATEGY"] == "LR" and state.timestamp > self.PRODUCTS[symbol]["LR_SIZE"] * 100:
            #     orders.extend(self.execute(symbol, order_depth, state))
            #     orders = [x for x in orders if x is not None]

            ### Store prices + quantities
            if len(order_depth.buy_orders) != 0 and len(order_depth.sell_orders) != 0:
                observed_price = statistics.mean([self.weighted_average_price(order_depth.buy_orders), self.weighted_average_price(order_depth.sell_orders)])
    ...
    def submit_order(self, product, price, quantity, state):
        if product in state.position.keys():
            can_buy = self.PRODUCTS[product]["PLIMIT"] - state.position[product]
            can_sell = state.position[product] + self.PRODUCTS[product]["PLIMIT"]
        else:
    ...
    def compute_price_regression(self, product, time):
