...
class Trader:
    ...
    PRODUCTS = {
        ...
        'COCONUT':   {"PLIMIT": 300, "CACHE": []},
        'COCONUT_COUPON':   {"PLIMIT": 600, "CACHE": []},
    }
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for symbol in state.order_depths:
            ...
            ### COCONUTS       -       LR?
            if symbol == "COCONUT":
                observed_price = np.mean([list(order_depth.buy_orders.items())[0], list(order_depth.sell_orders.items())[0]])
                self.PRODUCTS["COCONUT"]["CACHE"].append(observed_price)

                logger.print(str(self.PRODUCTS["COCONUT"]["CACHE"]))


                if len(self.PRODUCTS["COCONUT"]["CACHE"]) > 30:
                    long_ma = np.mean(self.PRODUCTS["COCONUT"]["CACHE"][-30:])
                    short_ma = np.mean(self.PRODUCTS["COCONUT"]["CACHE"][-10:])
                    ...
                    ### Pushing Orders
                    if short_ma > long_ma:
                        if state.position.get(symbol) is None:
                            if ask <= short_ma:
                                    orders.append(self.submit_order(symbol, ask, ask_amount, state))
                        else:
                            if state.position.get(symbol) < self.PRODUCTS[symbol]["PLIMIT"] // 4:
                                if ask <= short_ma - 1:
                                    orders.append(self.submit_order(symbol, ask, ask_amount, state))
                            elif state.position.get(symbol) < self.PRODUCTS[symbol]["PLIMIT"] // 2:
                                if ask <= short_ma - 5:
                                    orders.append(self.submit_order(symbol, ask, ask_amount, state))
                    else:
                        if state.position.get(symbol) is None:
                            ...
                            if state.position.get(symbol) > - self.PRODUCTS[symbol]["PLIMIT"] // 4:
                                ...
                                if bid >= short_ma + 5:
                                    orders.append(self.submit_order(symbol, bid, bid_amount, state))
                    logger.print(short_ma)

            if symbol == "COCONUT_COUPON" and False:
                if len(self.PRODUCTS["COCONUT"]["CACHE"]) > 0:
                    # if len(self.PRODUCTS["COCONUT"]["CACHE"]) % 23 == 0:
                    #     pct_change = (self.PRODUCTS["COCONUT"]["CACHE"][-1] - self.PRODUCTS["COCONUT"]["CACHE"][-23]) / self.PRODUCTS["COCONUT"]["CACHE"][-23]
                    #     self.PRODUCTS["COCONUT_COUPON"]["CACHE"].append(pct_change)
                    # if len(self.PRODUCTS["COCONUT_COUPON"]["CACHE"]) < 5:
                    #     calculated_price = self.price_option(self.PRODUCTS["COCONUT"]["CACHE"][-1], state, 0)
                    # else:
                    #     volatility = np.std(self.PRODUCTS["COCONUT_COUPON"]["CACHE"][-5]) * 15.8745078664 * 100
                    calculated_price = self.price_option(self.PRODUCTS["COCONUT"]["CACHE"][-1], state)
                    ...
                    ### Pushing Orders
                    if ask <= calculated_price:
                        orders.append(self.submit_order(symbol, ask, ask_amount, state))
                    if bid >= calculated_price:
                        orders.append(self.submit_order(symbol, bid, bid_amount, state))
    ...
    def price_option(self, underlying, state):
        S = underlying
        K = 10000
        T = (247 - state.timestamp / 1000000) / 250
        sigma = 0.157275

        d1 = (np.log(S / K) + (0.5 * sigma**2) * T) / (sigma * np.sqrt(T))
        d2 = d1 - sigma * np.sqrt(T)

        price = S * NormalDist(mu=0, sigma=1).cdf(d1) - K * NormalDist(mu=0, sigma=1).cdf(d2)
        logger.print(price)
        return price
